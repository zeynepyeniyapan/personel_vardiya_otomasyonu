        ...
        public class vardiyalar
        {
            ...
            public bool IzinKontrol(int sicilNo, DateTime tarih)
            {
                using (var baglan = new MySqlConnection(dbConnectionString))
                {
                    ...
                    string sorgu = $"SELECT COUNT(*) FROM tbl_izin WHERE sicil_no = '{sicilNo}' AND baslangic_tarih = '{tarih.ToString("yyyy-MM-dd")}'";
			...
			// Türkçe gün adlarına çevirme fonksiyonu
			private string GünAdiniCevir(string ingilizceGünAdi)
			{
				DateTimeFormatInfo dtfi = new CultureInfo("tr-TR", false).DateTimeFormat;
				DayOfWeek day = (DayOfWeek)Enum.Parse(typeof(DayOfWeek), ingilizceGünAdi, true);
			...
			public bool TatilKontrol(int sicilNo, DateTime tarih)
			{
				using (var baglan = new MySqlConnection(dbConnectionString))
				{
					...
					// Türkçe gün adı al
					string bugununGunu = tarih.ToString("dddd");
